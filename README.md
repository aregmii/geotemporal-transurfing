...
Every event stands on the globe where it happened, on a beam straight up from its own coordinates; two events in one place are two holograms at two heights. Time is a film: it rolls from the moment the page opens, and the transport at the bottom rewinds, pauses and fast-forwards it — a step faster each press, either way. The globe shows what was happening in the month NOW names: a day's event appears in its month and is gone the next; a war stays as long as it runs. Where a place is too crowded to draw them all, the top card wears a **+N** chip and lists the rest.
